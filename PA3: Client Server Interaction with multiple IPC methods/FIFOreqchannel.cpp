...
FIFORequestChannel::FIFORequestChannel(const string _name, const Side _side) : RequestChannel(_name, _side){ //base class consructor
	s1 = "fifo_" + my_name + "1";
	s2 = "fifo_" + my_name + "2";
		
	if (_side == SERVER_SIDE){
		wfd = open_ipc(s1, O_WRONLY | O_CREAT);
		rfd = open_ipc(s2, O_RDONLY | O_CREAT);
	}
	else{
		rfd = open_ipc(s1, O_RDONLY | O_CREAT);
		wfd = open_ipc(s2, O_WRONLY | O_CREAT);
	}
